...
import { Siren, ChevronRight, Zap, Volume2, VolumeX, Radio, ChevronLeft, Sparkles } from 'lucide-react';
      ...
      <div className="bg-slate-900 text-slate-300 px-4 py-2 border-b border-slate-800 text-xs">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-white">Live Urgent Network Standby</span>
            <span className="hidden sm:inline text-slate-400">· 0 active critical emergencies in your immediate dispatch zone</span>
          </div>
  ...
  return (
    <div className="bg-gradient-to-r from-red-700 via-red-600 to-rose-700 text-white px-4 py-2.5 shadow-md relative z-30 border-b border-red-800">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2.5 text-xs sm:text-sm">
        ...
        {/* Left Side: Live Indicator & Urgent Alert Details */}
        ...
        <div className="flex items-center gap-2 shrink-0 w-full md:w-auto justify-end">
          ...
          >
            All Alerts ({criticalRequests.length})
            <ChevronRight className="w-3 h-3" />
          </button>

          {/* Sound & Live Simulation Controls */}
          <div className="flex items-center gap-1 border-l border-red-500/40 pl-2 ml-1">
            <button
              onClick={toggleSound}
              className={`p-1.5 rounded-lg transition-colors ${
                isSoundEnabled ? 'bg-red-800/80 text-white hover:bg-red-800' : 'bg-red-950/40 text-red-300 hover:text-white'
              }`}
              title={isSoundEnabled ? 'Mute Urgent Alert Sound' : 'Enable Urgent Alert Sound'}
            >
            ...
            <button
              onClick={simulateIncomingEmergency}
              className="p-1.5 bg-red-800/80 hover:bg-red-800 rounded-lg text-white transition-colors flex items-center gap-1 text-xs font-semibold"
              title="Trigger a new Live Urgent Alert Simulation"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-300" />
              <span className="hidden xl:inline text-[11px]">Simulate</span>
            </button>
          </div>
        </div>
      </div>
